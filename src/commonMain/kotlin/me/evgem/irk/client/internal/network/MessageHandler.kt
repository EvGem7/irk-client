...
private class DefaultMessageHandler(
    ...
    private suspend fun readMessage(): Message {
        var arr = readMessageByteArray().run {
            copyOfRange(fromIndex = 0, toIndex = size - 2) // remove CRLF
        }
        ...
        val command = arr
            ...
            .let { if (it == -1) arr.lastIndex else it }
            .let { commandEndIndex ->
                arr.decodeToString(startIndex = 0, endIndex = commandEndIndex).also {
                    arr = arr.copyOfRange(fromIndex = commandEndIndex + 1, toIndex = arr.size)
                }
            }

        arr.split(Byte.Space)
    }

    private suspend fun readMessageByteArray(): ByteArray {
        var arr = ByteArray(0)
        var desiredSize = 2 // min message size (CRLF)
        do {
            readChannel.read(desiredSize = desiredSize) { source, start, endExclusive ->
                Log("read start=$start endExclusive=$endExclusive")
                var crFlag = false
                var toRead = 0
                for (i in start until endExclusive) {
                    if (crFlag && source[i] == Byte.LF) {
                        toRead = i.minus(start).plus(1).toInt()
                        break
                    } else {
                        crFlag = source[i] == Byte.CR
                    }
                }
                if (toRead > 0) {
                    arr = ByteArray(toRead)
                    source.loadByteArray(start, arr)
                }
                desiredSize += endExclusive.minus(start).toInt()
                return@read toRead
            }
        } while (arr.isEmpty())
        return arr
